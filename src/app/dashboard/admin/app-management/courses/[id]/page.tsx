...
export default function CourseDetailPage() {
  ...
  useEffect(() => {
    if (courseId && courseId !== "new") {
      fetchCourse();
      // Check if this course was imported from a playlist (non-critical, ignore errors)
      apiFetch(`/playlist/course/${courseId}`)
        .then(data => setPlaylistInfo(data as PlaylistInfo))
        .catch(() => {});
    } else {
      setLoading(false);
    }
  ...
  const syncPlaylist = async () => {
    setSyncing(true);
    try {
      const result = await apiFetch(`/playlist/course/${courseId}/sync`, { method: "POST" }) as {
        stats: { added: number; updated: number; unchanged: number };
      };
      const { added, updated } = result.stats;
      toast.success(
        added > 0 || updated > 0
          ? `Sync complete: ${added} new lecture${added !== 1 ? "s" : ""} added, ${updated} updated`
  ...
  return (
    <Protected requiredRole="admin">
      <main className="p-6 font-poppins min-h-screen">
        ...
        {/* Playlist Sync Panel — shown only for playlist-imported courses */}
        {playlistInfo && (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            className="mt-4 bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-4"
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-red-600 truncate mt-0.5">
                {playlistInfo.playlistTitle} &middot; {playlistInfo.channelName} &middot; {playlistInfo.importedVideoCount} lectures
              </p>
              <p className="text-xs text-red-500 mt-0.5">
                Last synced: {new Date(playlistInfo.lastSyncedAt).toLocaleString()}
                {playlistInfo.syncStatus === "error" && (
                  <span className="ml-2 text-red-700">— Sync error: {playlistInfo.syncError}</span>
                )}
              </p>
            </div>
            <button
              ...
              disabled={syncing}
              className="flex-shrink-0 flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-medium hover:bg-red-700 transition disabled:opacity-50"
            >
              {syncing ? (
                <>
